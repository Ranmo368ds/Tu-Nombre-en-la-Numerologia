...
export default function TaxMarketingSystem() {
    const t = useTranslations('TaxServicePage.marketing');
                ...
                <div className="max-w-6xl mx-auto space-y-12">
                    {pillars.map(({ id, icon: Icon, color, bgColor, borderColor }) => (
                        <div key={id} className={`${bgColor} rounded-2xl p-8 md:p-12 border-2 ${borderColor} shadow-lg hover:shadow-xl transition-shadow duration-300`}>
                            ...
                            <div className="mb-6">
                                ...
                                <ul className="space-y-2">
                                    {[1, 2, 3, 4, 5].map((num) => {
                                        ...
                                        if (!useCase || useCase === `TaxServicePage.marketing.${id}.useCases.item${num}`) return null;
                                        return (
                            ...
                            {t(`${id}.whatWeDo.title`) !== `TaxServicePage.marketing.${id}.whatWeDo.title` && (
                                <div className="bg-white/60 rounded-xl p-6 border border-gray-200">
                                    ...
                                    <ul className="space-y-2">
                                        {[1, 2, 3, 4, 5].map((num) => {
                                            ...
                                            if (!item || item === `TaxServicePage.marketing.${id}.whatWeDo.item${num}`) return null;
                                            return (
                                                <li key={num} className="flex items-start gap-3">
