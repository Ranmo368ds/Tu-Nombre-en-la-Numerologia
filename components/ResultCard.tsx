...
import { NumberMeaning } from "@/data/meanings";
...
const icons: Record<string, any> = {
    "Camino de Vida": Star,
    "Destino": Shield,
    "Deseo del Alma": Heart,
    "Personalidad": Zap,
};

export function ResultCard({ label, result, meaning, delay = 0 }: ResultCardProps) {
        ...
        >
            <Card className="glass-card border-none overflow-hidden h-full flex flex-col group">
                <CardHeader className="pb-2">
                    ...
                    <CardTitle className="font-mystic text-2xl text-slate-100 tracking-tight">{meaning.title}</CardTitle>
                </CardHeader>
                <CardContent className="space-y-5 text-sm md:text-md text-slate-300 flex-grow leading-relaxed">
                    <div className="relative">
                        <p className="italic text-slate-200 border-l-2 border-gold/30 pl-4 py-1">"{meaning.personality}"</p>
                    </div>

                    <div className="grid grid-cols-1 gap-5">
                        <section>
                            <h4 className="text-gold text-[11px] uppercase font-bold tracking-[0.1em] mb-2 flex items-center gap-2">
                                <span className="w-1 h-1 bg-gold rounded-full" /> Fortalezas
                            </h4>
                            <p className="pl-3 border-l border-white/5">{meaning.strengths}</p>
                        </section>
                        <section>
                            <h4 className="text-gold text-[11px] uppercase font-bold tracking-[0.1em] mb-2 flex items-center gap-2">
                                <span className="w-1 h-1 bg-gold rounded-full" /> Desafíos
                            </h4>
                            <p className="pl-3 border-l border-white/5">{meaning.challenges}</p>
                        </section>
                    </div>
                    ...
                    <div className="border-t border-white/10 pt-5 mt-auto bg-gold/5 -mx-6 px-6 py-4">
                        <h4 className="text-gold text-[10px] uppercase font-bold tracking-[0.2em] mb-3">Sinergia Cósmica</h4>
                        <div className="space-y-2">
                            <p className="flex items-start gap-2 text-sm">
                                <Heart className="w-3.5 h-3.5 mt-0.5 text-pink-400/60" />
                                <span><strong className="text-slate-100">Amor:</strong> {meaning.love}</span>
                            </p>
                            <p className="flex items-start gap-2 text-sm">
                                <Briefcase className="w-3.5 h-3.5 mt-0.5 text-blue-400/60" />
                                <span><strong className="text-slate-100">Carrera:</strong> {meaning.career}</span>
                            </p>
                        </div>
                    </div>
                </CardContent>
