...
import { ResultCard } from "@/components/numerology/ResultCard";
import {
    calculateLifePath,
...
import { CosmicBackground } from "@/components/numerology/CosmicBackground";
import { Tabs, TabsContent, TabsList, TabsTrigger } from "@/components/ui/tabs";
import { CompatibilitySection } from "@/components/numerology/CompatibilitySection";
import { HomeBusinessSection } from "@/components/HomeBusinessSection"; // Keep for now or move to specialized if it grows
import { CyclesSection } from "@/components/numerology/CyclesSection";
import { LanguagePicker } from "@/components/numerology/LanguagePicker";
        ...
        <main className="relative min-h-screen flex flex-col overflow-x-hidden">
            ...
            <div className="container mx-auto px-4 py-8 md:py-16 relative z-10 flex-grow flex flex-col items-center">
                ...
                <div className="absolute top-4 left-4 z-50">
                    <Link href="/" className="text-white/60 hover:text-gold transition-colors flex items-center gap-2">
                        ← {t('title').split(' ')[0]} Home
                    </Link>
                </div>
                        ...
                        <TabsContent value="main" className="space-y-12">
                            ...
                            <AnimatePresence>
                                {results && (
                                    ...
                                    >
                                        <ResultCard label="life_path" result={results.lifePath} delay={0.1} />
                                        <ResultCard label="destiny" result={results.destiny} delay={0.2} />
                                        <ResultCard label="soul_urge" result={results.soulUrge} delay={0.3} />
                                        <ResultCard label="personality" result={results.personality} delay={0.4} />
                                    </motion.div>
                                )}
                            </AnimatePresence>
                        </TabsContent>
